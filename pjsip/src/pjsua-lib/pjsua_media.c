...
pj_status_t pjsua_media_subsys_init(const pjsua_media_config *cfg)
{
	...
	status = pjmedia_endpt_create(&pjsua_var.cp.factory,
								  pjsua_var.media_cfg.has_ioqueue ? NULL : pjsip_endpt_get_ioqueue(pjsua_var.endpt),
	...
	pj_bool_t async)
{
	...
	if (ice_cfg.stun_tp_cnt)
	{
		...
		for (i = 0; i < ice_cfg.stun_tp_cnt; ++i)
		{
			...
			pj_sockaddr_init(ice_cfg.stun_tp[i].af,
							 ...
							 (ice_cfg.stun_tp[i].af == pj_AF_INET() ? &cfg->bound_addr : &IN6_ADDR_ANY),
	...
	if (acc_cfg->turn_cfg.enable_turn)
	{
		...
		for (i = 0; i < ice_cfg.turn_tp_cnt; i++)
		{
			...
			pj_sockaddr_init(ice_cfg.turn_tp[i].af,
							 ...
							 (ice_cfg.turn_tp[i].af == pj_AF_INET() ? &cfg->bound_addr : &IN6_ADDR_ANY),
							 (pj_uint16_t)cfg->port);
	...
	if (!async && !trickle)
	{
		...
		pjsip_dialog *dlg = call_med->call->inv ? call_med->call->inv->dlg : NULL;
		if (has_pjsua_lock)
			PJSUA_UNLOCK();
		if (dlg)
		{
									  ...
									  int *sip_err_code)
{
	...
	if (!call_med->tp_orig)
	{
		...
		if (pjsua_var.ua_cfg.cb.on_create_media_transport_srtp)
		{
			...
			(*pjsua_var.ua_cfg.cb.on_create_media_transport_srtp)(call->index, call_med->idx, &srtp_opt2);
										 ...
										 const pjsua_med_tp_state_info *info)
{
	...
	for (mi = 0; mi < call->med_prov_cnt; ++mi)
	{
		...
		if (call_med->tp && call_med->tp_st == PJSUA_MED_TP_IDLE)
		{
			...
			if (!tmp_pool)
			{
				tmp_pool = (call->inv ? call->inv->pool_prov : call->async_call.dlg->pool);
			}
			...
			if (call_med->use_custom_med_tp)
			{
				...
				call_med->tp =
					(*pjsua_var.ua_cfg.cb.on_create_media_transport)(call_id, mi, call_med->tp,
																	 custom_med_tp_flags);
				if (!call_med->tp)
				{
					status =
			...
			if (call_med->tp)
			{
				unsigned options = (call_med->enable_rtcp_mux ? PJMEDIA_TPMED_RTCP_MUX : 0);
									 ...
									 pjsua_med_tp_state_cb cb)
{
	...
	if (async)
	{
		pj_pool_t *tmppool = (call->inv ? call->inv->pool_prov : call->async_call.dlg->pool);
	...
	for (mi = 0; mi < call->med_prov_cnt; ++mi)
	{
		...
		if (enabled)
		{
			...
			status = pjsua_call_media_init(call_med, media_type,
										   ...
										   async,
										   (async ? &media_channel_init_cb : NULL));
...
pj_status_t pjsua_media_channel_init2(pjsua_call_id call_id,
									  pjsip_role_e role,
									  int security_level,
									  pj_pool_t *tmp_pool,
									  const pjmedia_sdp_session *rem_sdp,
									  ...
									  pjsua_med_tp_state_cb cb)
{
	...
	for (mi = 0; mi < call->med_prov_cnt; ++mi)
	{
		...
		{
			/* Initialize default initial media direction as bidirectional -- Kaan changed to Decode*/
			call_med->def_dir = PJMEDIA_DIR_DECODING;
		}
		call_med->dir = call_med->def_dir;
		...
		/* Setup RTCP-FB */
		{
			...
			else
			{
				...
				rtcp_cfg.dont_use_avpf = acc->cfg.rtcp_fb_cfg.dont_use_avpf ||
										 (acc->cfg.rtcp_fb_cfg.cap_count == 0 && rtcp_cfg.cap_count == 0);
			}
									   ...
									   const pjmedia_sdp_session *remote_sdp)
{
	...
	for (mi = 0; mi < call->med_cnt; ++mi)
	{
		...
		if (call_med->type == PJMEDIA_TYPE_AUDIO)
		{
			...
			{
				si->param->setting.frm_per_pkt = (pj_uint8_t)(pjsua_var.media_cfg.ptime / si->param->info.frm_ptime);
 ...
 */
PJ_DEF(pj_status_t)
pjsua_enum_codecs(pjsua_codec_info id[],
				  unsigned *p_count)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsua_codec_set_priority(const pj_str_t *codec_id,
						 pj_uint8_t priority)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsua_codec_get_param(const pj_str_t *codec_id,

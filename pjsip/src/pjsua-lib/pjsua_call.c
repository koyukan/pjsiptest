 ...
 */
PJ_DEF(pj_status_t)
pjsua_enum_calls(pjsua_call_id ids[],
				 unsigned *count)
{
	unsigned i, c;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_make_call2(pjsua_acc_id acc_id,
					  const pj_str_t *dest_uri,
					  const pjsua_call_setting *opt,
					  void *user_data,
					  const pjsua_msg_data *msg_data,
					  pjsua_call_id *p_call_id)
{
	...
	{
		/* Init media channel */
		status = pjsua_media_channel_init2(call->index, PJSIP_ROLE_UAC,
										   call->secure_level, dlg->pool,
										   NULL, NULL, PJ_TRUE,
										   &on_make_call_med_tp_complete);
	}
							...
							pjmedia_sdp_session **p_sdp)
{
	...
	for (; i < rem_sdp->media_count; ++i)
	{
		...
		pj_sockaddr_init(med_use_ipv4 ? PJ_AF_INET : PJ_AF_INET6,
						 ...
						 med_use_ipv4 ? pj_strset2(&tmp_st, "127.0.0.1") : pj_strset2(&tmp_st, "::1"),
 ...
 */
PJ_DEF(pj_bool_t)
pjsua_call_is_active(pjsua_call_id call_id)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_get_info(pjsua_call_id call_id,
					pjsua_call_info *info)
{
	...
	for (mi = 0; mi < call->med_cnt &&
				 info->media_cnt < PJ_ARRAY_SIZE(info->media);
		 ++mi)
	{
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_set_user_data(pjsua_call_id call_id,
						 void *user_data)
{
	PJ_ASSERT_RETURN(call_id >= 0 && call_id < (int)pjsua_var.ua_cfg.max_calls,
					 PJ_EINVAL);
 ...
 */
PJ_DEF(void *)
pjsua_call_get_user_data(pjsua_call_id call_id)
{
	PJ_ASSERT_RETURN(call_id >= 0 && call_id < (int)pjsua_var.ua_cfg.max_calls,
					 NULL);
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_get_rem_nat_type(pjsua_call_id call_id,
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_answer(pjsua_call_id call_id,
				  unsigned code,
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_hangup(pjsua_call_id call_id,
				  unsigned code,
				  const pj_str_t *reason,
				  const pjsua_msg_data *msg_data)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_process_redirect(pjsua_call_id call_id,
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_set_hold(pjsua_call_id call_id,
					const pjsua_msg_data *msg_data)
{
...
PJ_DEF(pj_status_t)
pjsua_call_set_hold2(pjsua_call_id call_id,
					 unsigned options,
					 const pjsua_msg_data *msg_data)
{
	pjmedia_sdp_session *sdp;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_reinvite(pjsua_call_id call_id,
					unsigned options,
					const pjsua_msg_data *msg_data)
{
	pjsua_call *call;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_reinvite2(pjsua_call_id call_id,
					 const pjsua_call_setting *opt,
					 const pjsua_msg_data *msg_data)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_update(pjsua_call_id call_id,
				  unsigned options,
				  const pjsua_msg_data *msg_data)
{
	pjsua_call *call;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_update2(pjsua_call_id call_id,
				   const pjsua_call_setting *opt,
				   const pjsua_msg_data *msg_data)
{
	pjmedia_sdp_session *sdp = NULL;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_xfer(pjsua_call_id call_id,
				const pj_str_t *dest,
				const pjsua_msg_data *msg_data)
{
	pjsip_evsub *sub;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_xfer_replaces(pjsua_call_id call_id,
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_send_im(pjsua_call_id call_id,
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_send_typing_ind(pjsua_call_id call_id,
						   pj_bool_t is_typing,
						   const pjsua_msg_data *msg_data)
{
	pjsua_call *call;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_send_request(pjsua_call_id call_id,
...
static void trickle_ice_recv_sip_info(pjsua_call *call, pjsip_rx_data *rdata)
{
	...
	for (i = 0; i < sdp_info->sdp->media_count; ++i)
	{
		...
		for (j = 0; j < med_cnt; ++j)
		{
			pjsua_call_media *cm = use_med_prov ? &call->media_prov[j] : &call->media[j];
			tp = cm->tp_orig;
									  ...
									  struct pj_timer_entry *te)
{
	...
	if (!forced)
	{
		for (i = 0; i < med_cnt; ++i)
		{
			pjsua_call_media *cm = use_med_prov ? &call->media_prov[i] : &call->media[i];
